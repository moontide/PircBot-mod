...
public abstract class PircBot implements ReplyConstants
{
	 ...
	 */
	public static final String VERSION = "1.5.0-mod";

	public static final SimpleDateFormat _LOG_DATE_FORMAT = new SimpleDateFormat ("yyyy-MM-dd HH:mm:ss.SSS");
	public static final Date _LOG_DATE = new Date ();
	...
	public PircBot ()
	{
		AutoDetectAndSetProxy ();
	}

	/**
	 * Auto detect HTTP proxy and SOCKS proxy from system properties of.
	 * If both HTTP and SOCKS properties are set, it will use HTTP proxy.
	 */
	void AutoDetectAndSetProxy ()
	{
		if (System.getProperty ("http.proxyHost") != null && !System.getProperty ("http.proxyHost").isEmpty () || System.getProperty ("http.proxyPort") != null && !System.getProperty ("http.proxyPort").isEmpty ())
		{
			...
			_proxyPort = Integer.parseInt (System.getProperty ("http.proxyPort"));
			_proxyUser = System.getProperty ("http.proxyUser");
			_proxyPassword = System.getProperty ("http.proxyPassword");
			log ("HTTP Proxy detected: " + _proxyHost + ":" + _proxyPort);
			if (_proxyUser != null)
			{
				Authenticator.setDefault (new Authenticator ()
				{
					...
					{
						return new PasswordAuthentication (_proxyUser, _proxyPassword.toCharArray ());
					}
				});
			}
			SocketAddress proxyAddr = new InetSocketAddress (_proxyHost, _proxyPort);
			_proxy = new Proxy (Proxy.Type.HTTP, proxyAddr);
		}
		else if (System.getProperty ("socksProxyHost") != null && !System.getProperty ("socksProxyHost").isEmpty () || System.getProperty ("socksProxyPort") != null && !System.getProperty ("socksProxyPort").isEmpty ())
		{
			_proxyHost = System.getProperty ("socksProxyHost");
			_proxyPort = Integer.parseInt (System.getProperty ("socksProxyPort"));
			log ("SOCKS Proxy detected: " + _proxyHost + ":" + _proxyPort);
			SocketAddress proxyAddr = new InetSocketAddress (_proxyHost, _proxyPort);
			_proxy = new Proxy (Proxy.Type.SOCKS, proxyAddr);
		}
	}

	public Socket CreateSocketForHTTPProxy () throws UnknownHostException, IOException
	{
		Socket socket = null;
		socket = new Socket (_proxyHost, _proxyPort);
		StringBuilder request = new StringBuilder ();
		request.append ("CONNECT " + _server + ":" + _port + " HTTP/1.0\r\n");

		if (_proxyUser != null && !_proxyUser.isEmpty ())
		{
			String proxyPass = _proxyUser + ":" + _proxyPassword;

			// Use apache codec library dynamically (at runtime, not compilation
			// time)
			Class codec_base64;
			java.lang.reflect.Method encode_base64;
			try
			{
				codec_base64 = Class.forName ("org.apache.commons.codec.binary.Base64");
				encode_base64 = codec_base64.getMethod ("encodeBase64String", byte[].class);
				String proxyPassInBase64 = (String) encode_base64.invoke (codec_base64, proxyPass.getBytes ());
				request.append ("Proxy-Authentication: Basic ");
				request.append (proxyPassInBase64);
				request.append ("\r\n");
			}
			catch (Exception e)
			{
		...
		while (true)
		{
			...
			log ("Proxy response head #" + nLineNO + " " + line);

			if (nLineNO == 1)
			{
				String[] statusLineElements = line.split (" +");
				String protocol = statusLineElements[0];
				String sStatusCode = statusLineElements[1];
				String sStatusMsg = statusLineElements[2];
				int statusCode = Integer.parseInt (sStatusCode);
				if (statusCode != 200)
				{
					throw new RuntimeException ("Proxy server returns a non-successful status line: [" + line + "]. Does your proxy support CONNECT method?");
				}
			}
			else if (line.isEmpty ())
			{
				log ("Proxy response heads have been consumed.");
				break; // HTTP Head is consumed
			}
		}
		return socket;
	}
	 ...
	 * password. The onConnect method is called upon success. <br/>
	 * <h3>代理支持</h3>
	 * 如果要用 HTTP 代理连接 IRC 服务器，则需要通过 java -D 设置以下系统属性
	 * <dl>
	 * <dt><code>http.proxyHost</code></dt>
	 * <dd>http 代理服务器主机地址。必须设置</dd>
	 * <dt><code>http.proxyPort</code></dt>
	 * <dd>http 代理服务器端口。必须设置</dd>
	 * <dt><code>http.proxyUser</code></dt>
	 * <dd>http 代理服务器用户名。可选设置</dd>
	 * <dt><code>http.proxyPassword</code></dt>
	 * <dd>http 代理服务器密码。可选设置</dd>
	 * </dl>
	 * 如果要用 SOCKS 代理连接 IRC 服务器，则需要通过 java -D 设置以下系统属性
	 * <dl>
	 * <dt><code>socksProxyHost</code></dt>
	 * <dd>SOCKS 代理服务器主机地址。必须设置</dd>
	 * <dt><code>socksProxyPort</code></dt>
	 * <dd>SOCKS 代理服务器端口。必须设置</dd>
	 * </dl>
	 *
	 * 如果同时设置了 HTTP 代理和 SOCKS 代理，则 HTTP 代理优先。
	 *
	 * @param hostname
	 *            The hostname of the server to connect to.
	 * @param port
	...
	public final synchronized void connect (String hostname, int port, String password) throws IOException, IrcException, NickAlreadyInUseException
	{
		...
		if (_proxy != null)
		{
			if (_proxy.type ().equals (Proxy.Type.HTTP))
			{
				socket = CreateSocketForHTTPProxy ();
			}
			else
			{
				socket = new Socket (_proxy);
		...
		this.log ("*** Connected to server or proxy.");
	...
	public void log (String line)
	{
		if (_verbose)
		{
			_LOG_DATE.setTime (System.currentTimeMillis ());
			System.out.println (_LOG_DATE_FORMAT.format (_LOG_DATE) + " " + line);
		}
	}
	...
	public final boolean toggleVerbose ()
	{
		_verbose = !_verbose;
		return _verbose;
	}
	...
	private String _name = "PircBot-mod";
	private String _nick = _name;
	private String _login = "PircBot-mod";
	private String _version = "PircBot-mod " + VERSION + " Java IRC Bot - www.jibble.org, with modification by LiuYan. Added feature: connect via HTT proxy";
